...
- `app/components/cache-lab.tsx` — the interactive Cache tab: prefix bar (per-tier warm/cold), TTL clock, invalidation buttons, cost ledger, "walk away 3h" preset. Time advances via Wait buttons + Send, OR a **live ▶ Play** mode (`setInterval`, ~12× sim-time) that auto-pauses the instant the cache goes cold.
